...
import type {LiteralString, IndexedSectionListProps, SectionItem, IndexedSectionListData} from './types';
  ...
  <TitleKey extends LiteralString<TitleKey>, UniqueKey extends LiteralString<UniqueKey>>(
    ...
    const sections = React.useMemo<SectionListData<SectionItem<TitleKey, UniqueKey>>[]>(() => {
      ...
      for (const item of items) {
        ...
        let itemKey: string | number | undefined = typeof item === 'string' ? undefined : item[uniqueKey];
      ...
      return Object.entries(itemSections)
        .map<IndexedSectionListData<TitleKey, UniqueKey>>(([sectionKey, unsortedData]) => ({
          title: sectionKey,
          key: sectionKey,
          data: unsortedData.sort((leftItem, rightItem) => compareStrings(leftItem.title, rightItem.title)),
          ...(getSectionProps ? getSectionProps(sectionKey, unsortedData) : {}),
        }))
        .sort((leftSection, rightSection) => compareStrings(leftSection.title, rightSection.title));
