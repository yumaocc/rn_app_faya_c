...
import {StyleSheet, View, Animated, FlatList, Easing, GestureResponderEvent, ListRenderItemInfo} from 'react-native';
...
export default ({indexTextStyle, indexes, onSelectIndex, indexItemHeight, scrollEfficiency, wrapperStyle}: IndexListProps) => {
    ...
    () =>
      ({value}) => {
        ...
        if (cursorPosition.current !== null) {
          ...
          let newSelectedIndex = Math.floor(((cursorPosition.current + flatListScroll.current) / listHeight) * indexes.length);
  ...
  const animateScroll = React.useCallback(
    (position: number) => {
      const scrollDuration = (Math.abs(position - flatListScroll.current) * 1000) / SCROLL_PIXELS_PER_SECOND;
      ...
      <View
        ...
        <Animated.FlatList
          ...
          onResponderMove={(event: GestureResponderEvent) => {
            ...
            cursorPosition.current = Math.max(0, Math.min(containerHeight, event.nativeEvent.pageY - containerPageY));
            if (cursorPosition.current !== null && containerHeight < listHeight) {
              ...
              if (cursorPosition.current === 0) {
                ...
                scrollPosition.setValue(
                  Math.max(0, Math.min(listHeight - containerHeight, flatListScroll.current + (cursorPrevPosition.current - cursorPosition.current) * efficiency)),
                );
              }
            } else {
          ...
          renderItem={({item, index}: ListRenderItemInfo<string>) => (
            <IndexItem index={index} item={item} onSelectIndex={onSelectIndex} indexItemHeight={indexItemHeight} indexTextStyle={indexTextStyle} />
          )}
        />
      </View>
    </View>
